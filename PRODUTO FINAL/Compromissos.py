...
class Compromisso():
    ...
    def setFlexivel(self, duracao, prioridade, dataLimite, timeDas, timeAte, diasDaSemana):
        self.tipo = 'FLEXIVEL'
        self.duracao = (duracao.hour()*3600 + duracao.minute()*60)/62.06896551724138
    ...
    def setTempConfig(self, yInicio, yTermino, xInicio, inicioDate, inicioTime, terminoDate, terminoTime, retangMesInicio, retangMesTermino, retangSemanaInicio, retangSemanaTermino, dayOrder, repetir, allDay):
        self.repetir = repetir
        self.transporte = 'A Pé'
        self.allDay = allDay
    ...
    def setListDescricao(self):
        ...
        self.listDescricao.setSelectable(False)
    ...
    def setRetangDia(self):
        if self.allDay == False:
            if self.inicioDate == self.terminoDate:
                time = QTime(0,0)
                self.yInicio = 39 + time.secsTo(self.inicioTime)/62.06896551724138
                self.yTermino = 39 + time.secsTo(self.terminoTime)/62.06896551724138
